...
static void free_node(c_avl_node_t *avl_node,int free_key ,int free_value)
{
    if(avl_node == NULL)
        return;
...
void c_avl_destroy(c_avl_tree_t *avl_tree,int free_key, int free_value)
{
    if(avl_tree == NULL)
        return;

    free_node(avl_tree->root,free_key,free_value);
    free(avl_tree);
    avl_tree = NULL;
}
// sucess return 0 
// fail   return  !0 
static c_avl_node_t* create_avl_node(void *key, void *value)
{
    c_avl_node_t *new = (c_avl_node_t *)malloc(sizeof(c_avl_node_t));
    if (new == NULL){
...
int c_avl_insert(c_avl_tree_t *avl_tree,void *key,void *value)
{
    ...
    if(avl_tree == NULL || avl_tree->compare == NULL){
        printf("avl_tree is NULL or avl_tree->compare is NULL\n");
        return 1;
    }
...
static int _remove(c_avl_tree_t *avl_tree,c_avl_node_t *avl_node)
{
    ...
    //左右子树不为空,将要删除的节点移动到其左子树的最大节点或右子树的最小节点
    ...
    if((avl_node->left == NULL) && (avl_node->right == NULL))
    {
        ...
        free(avl_node);;
    }else if(avl_node->left == NULL)//左子树为空
    {
